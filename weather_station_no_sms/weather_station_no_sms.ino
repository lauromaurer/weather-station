/*
 * Weather Station — Full Sensor Suite + SD Logging + Low Power  v4.2 (no SMS)
 ...
 *   No modem / SMS — TinyGSM not required
 *
 * ── STATION_MODE ─────────────────────────────────────────────────────────────
 *   0 = TEST       reads every 3 s, no deep sleep
 *   1 = ECO        reads every 3 min with deep sleep between
 *   2 = PRODUCTION deep sleep SLEEP_MINUTES between reads
 *
 * ── Wiring ───────────────────────────────────────────────────────────────────
...
#define SLEEP_MINUTES  5          // used in PRODUCTION mode only (fixed interval fallback)
#define ECO_MINUTES    3          // fixed sleep for ECO mode (fixed interval fallback)

// ── Location (for sunrise/sunset calculation) ─────────────────────────────────
#define STATION_LAT         42.76f   // Elba, Italy: 42.76f — positive = north
#define STATION_LON         10.29f   // Elba, Italy: 10.29f — positive = east

// ── Adaptive daylight schedule ────────────────────────────────────────────────
// The station wakes only during daylight, spreading DAILY_MEASUREMENTS evenly
// between (sunrise + SUNRISE_BUFFER_MIN) and (sunset - SUNSET_BUFFER_MIN).
// Applies to ECO and PRODUCTION modes only; TEST mode always reads every 3 s.
#define DAILY_MEASUREMENTS   30    // how many readings per day
#define SUNRISE_BUFFER_MIN  30    // skip this many minutes after sunrise (low sun angle)
#define SUNSET_BUFFER_MIN   5    // stop this many minutes before sunset

// ── Session start date & time ─────────────────────────────────────────────────
// Set these to the moment you power on the station for each deployment.
// Used to derive current date (for sunrise calc) and current time of day.
#define SESSION_START_YEAR  2026
#define SESSION_START_MONTH    7
#define SESSION_START_DAY      16
#define SESSION_START_HOUR     16
#define SESSION_START_MIN      30 // actual start at 16:15
...
#define BOOST_EN      33
#define BAT_PIN       35
#define SOLAR_PIN     36   // solar panel voltage (onboard voltage divider, same ratio as BAT)
#define LED_BLUE      12   // blue user LED on T-SIM7000G (active LOW: HIGH=off, LOW=on)
// NOTE: green/red LEDs are hardwired to the charging IC and SIM7000G — not software controlled.
...
// ── Session timestamp (survives deep sleep) ───────────────────────────────────
// accumulatedMs counts total elapsed session time across all sleep/wake cycles.
// On each wakeup the MCU adds the sleep interval it just completed, so the
// value always reflects "how long since the very first boot of this session".
// firstBoot distinguishes the very first power-on from subsequent wakeups.
RTC_DATA_ATTR unsigned long accumulatedMs = 0;
RTC_DATA_ATTR bool          firstBoot     = true;
RTC_DATA_ATTR uint32_t      lastSleepMs   = 0;   // actual sleep duration of last cycle
                                                  // (variable now — depends on schedule)
...
// Returns the session-elapsed time in ms — used as the CSV timestamp.
// TEST mode: board never sleeps, so millis() is naturally correct.
// ECO / PRODUCTION: use the RTC-memory accumulator instead.
...
// Returns true when the solar panel is actively delivering power.
// Threshold of 0.5 V filters out ADC noise when the panel is disconnected.
bool solarCharging() {
  return getSolarVoltage() > 0.5f;
}
...
void sdLog(float insideT, float insideH, float pressure,
  ...
  f.print(',');
  f.print(batPct);      f.print(',');
  f.print(solarV, 2);   f.print(',');
  f.println(solarOn ? 1 : 0);
  f.close();
}
...
bool isEuropeanDST(int year, int month, int day) {
  ...
  // Find last Sunday of the month using Tomohiko Sakamoto's weekday algorithm
...
void sunriseSunset(int doy, float lat, float lon, float tzHours,
  ...
  // Hour angle at horizon (90.833° accounts for refraction + solar disk radius)
  float cosHA = cosf(90.833f * DEG) / (cosf(lat * DEG) * cosf(decl))
              - tanf(lat * DEG) * tanf(decl);
  cosHA = constrain(cosHA, -1.0f, 1.0f); // clamp for polar regions
  float ha = acosf(cosHA);               // radians
...
int buildDaySchedule(int *slots, int maxSlots) {
  ...
  for (int i = 0; i < n; i++) {
    // Evenly space: i=0 → playStart, i=n-1 → playEnd
...
uint32_t minutesUntilNextWakeup() {
  int slots[DAILY_MEASUREMENTS];
  int count = buildDaySchedule(slots, DAILY_MEASUREMENTS);
  int nowMin = currentMinuteOfDay();

  // Find next slot later today (with a 2 min buffer so we don't re-trigger the same slot)
  ...
  // All today's slots are done — compute tomorrow's first slot
  int year, month, day;
  currentDate(year, month, day);
  // Advance one day
  day++;
...
#endif
#if   (STATION_MODE == 0)
  Serial.println("    [ TEST MODE — 3 s reads, no sleep ]\n");
#elif (STATION_MODE == 1)
  Serial.printf( "    [ ECO MODE  — %d min deep sleep ]\n\n", ECO_MINUTES);
#else
  Serial.printf( "    [ PRODUCTION — %d min deep sleep ]\n\n", SLEEP_MINUTES);
#endif

  // Release deep-sleep GPIO holds, then turn blue LED off (active LOW → HIGH = off)
...
void takeMeasurement() {
  // Quick double-blink on blue LED so you can see the station is active
  for (int i = 0; i < 2; i++) {
  ...
  Serial.printf("Solar panel             : %.2f V  [%s]\n",    solarV, solarOn ? "CHARGING" : "idle");
  ...
  sdLog(insideT, insideH, pressure, outsideT, outsideH, lux, w, batPct, solarV, solarOn);
